...
http {
    ...
    # Настроим получение сертификатов от Let's Encrypt через ACME. Для большей
...
Создадим также четыре проксируемых сервера. Запросы, поступающие к серверу Angie, будут проксироваться на эти сервера. Для простоты в качестве проксируемых серверов будем использовать web-приложение vscode/webdebugger, которое использовалось во время лекции. Проксируемые сервера будут различаться цветом фона выдаваемой странцы.
...
Сейчас у нас по умолчанию настроена балансировка Round Robin. При этом типе балансировке запросы от клиентов равномерно распределяются между серверами в циклическом порядке. Убедимся, что наш сервер работает именно так. Несколько раз обновим главную страницу, затем с помощью Console Light посмотрим, сколько всего запросов пришло на сервер Angie и сколько запросов было проксировано на каждый сервер web-приложений.
...
![round-robin-запросы-по-бэкендам](https://github.com/user-attachments/assets/4a0b7cc0-812a-42d5-b691-84ae2bc85850)

![round-robin-общее-количество-запросов](https://github.com/user-attachments/assets/551d20a4-d59a-4f6b-981e-c91cce701818)
</details>

#### Балансировка IP Hash

Изменим тип балансировки на IP Hash. При этом типе балансировке запросы распределяются по серверам на основе IP-адресов клиентов. В качестве ключа для хэширования используются первые три октета IP-адреса клиента. Метод гарантирует, что запросы одного и того же клиента будут всегда передаваться на один и тот же сервер.

В конфигурации Angie тип балансировки на IP Hash задаётся путём добавления директивы `ip_hash` в соответствующий блок `upstream`.
...
#### Конфигурация с отключением одного из бэкендов
...
#### Конфигурация с резервным бэкендом
