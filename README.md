...
Создадим также четыре проксируемых сервера. Запросы, поступающие к серверу Angie, будут проксироваться на эти сервера. Для простоты, в качестве проксируемых серверов будем использовать web-приложение vscode/webdebugger, которое спользовалось во время лекции. Проксируемые сервера будут различаться цветом фона выдаваемой странцы.
